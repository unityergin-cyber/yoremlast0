...
function AppNavigator() {
  ...
  // Çalışma saatlerini kontrol et - timeout ile
  const checkWorkingHours = async () => {
    try {
      ...
      if (error.name === 'AbortError') {
        console.log("Çalışma saatleri kontrolü zaman aşımına uğradı, varsayılan olarak AÇIK");
      } else {
  ...
  useEffect(() => {
    // Route'u hemen ayarla - AsyncStorage işlemlerini beklemeden
    console.log('🔵 Uygulama başlatılıyor...');
    setInitialRoute('Main');
    setIsLoading(false);
    
    // AsyncStorage işlemlerini arka planda yap (bloklamadan)
    const checkLoginStatus = async () => {
      try {
        // AsyncStorage işlemlerini paralel yap (daha hızlı)
        const [token, userId, telefon] = await Promise.all([
          AsyncStorage.getItem('userToken'),
          AsyncStorage.getItem('userId'),
          AsyncStorage.getItem('telefonNumarasi')
        ]);
    ...
    // AsyncStorage işlemlerini arka planda başlat
    checkLoginStatus();
    
    // Çalışma saatlerini arka planda kontrol et (bloklamadan)
    checkWorkingHours().catch(err => {
      console.error('Çalışma saatleri kontrolü arka planda başarısız:', err);
    });
